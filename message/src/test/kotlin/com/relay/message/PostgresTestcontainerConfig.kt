 ...
 * migrations are Postgres SQL (`uuid` columns, `timestamp with time zone`), and running the suite
 * against a different engine would have verified a schema no environment actually has. The trade
 * is that tests now need Docker — which the Kafka side of this suite does not, since
 * `@EmbeddedKafka` is in-JVM.
 *
